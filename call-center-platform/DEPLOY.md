...
|Variable|Value|
|-|-|
|`Jwt\\\_\\\_Key`|a long random secret (32+ chars) — do **not** reuse the dev key in `appsettings.json`|
|`Jwt\\\_\\\_Issuer`|`CallCenterPrototype` (or your own)|
|`Jwt\\\_\\\_Audience`|`CallCenterPrototypeClient`|
|`Cors\\\_\\\_AllowedOrigins\\\_\\\_0`|your Vercel URL, e.g. `https://call-center-platform.vercel.app`|
|`ConnectionStrings\\\_\\\_Default`|see the SQLite note below|
...
`/data`) and set `ConnectionStrings\\\_\\\_Default` to
...
Go back to Railway and make sure `Cors\\\_\\\_AllowedOrigins\\\_\\\_0` matches the **exact**
